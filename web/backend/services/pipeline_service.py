...
from agent_runtime.config import (
    ...
    video_api_key, video_base_url, video_model, video_provider,
)
...
def _suppress_pipeline_output():
    """Redirect stdout/stderr and mute noisy loggers during pipeline execution."""
    try:
        # Redirect stdout
...
class PipelineService:
    ...
    async def _run_planning(
        ...
        try:
            ...
            self._session_index.update_stage(session_id, "error", str(exc))
            await self._broadcast_ws(session_id, {
                "type": "pipeline_error", "session_id": session_id,
                "error": str(exc),
            })
        ...
        try:
            ...
            for idx, scene_script in enumerate(scene_scripts):
                ...
                with _suppress_pipeline_output():
                    await sp(
                        script=script_text,
                        user_requirement=self._session_index.get(session_id).get("user_requirement", ""),
                        style=self._session_index.get(session_id).get("style", ""),
                        characters=characters,
                        progress=self._ws_progress_callback(session_id),
                        quiet=True,
                    )
