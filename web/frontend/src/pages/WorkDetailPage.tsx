...
export default function WorkDetailPage() {
  ...
  const videoUrl = session.has_final_video ? getFileUrl(sessionId!, 'idea2video/final_video.mp4') : null
  ...
  const characterImages = characters.flatMap((c: any) => {
    ...
    const sid = sessionId!
    const charName = c.identifier || `角色${c.idx ?? ''}`
    const views = ['front', 'side', 'back']
    views.forEach(view => {
      // Construct character portrait URL pattern
      const url = getFileUrl(sid, `idea2video/characters/${charName}/${view}.png`)
    ...
    <div className="p-6 max-w-4xl mx-auto page-enter">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {isIncomplete && (
          <button
            ...
            className="flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-[#E84A4F] transition-colors"
          >
        ...
        <div className="mb-6 rounded-xl border bg-card p-5">
          ...
          <div className="flex gap-3 flex-wrap">
            {characters.map((c: any, i: number) => (
              <div key={i} className="rounded-lg bg-muted px-4 py-2.5 text-sm">
                <span className="font-semibold">{c.identifier || `角色${c.idx ?? ''}`}</span>
                {c.static_features && (
      ...
      {scenes.length > 0 && (
        <div className="mb-6 rounded-xl border bg-card p-5">
          ...
          <div className="space-y-1">
            {scenes.map((s: any) => (
              <div key={s.index} className="flex items-center justify-between py-2 px-3 rounded-lg text-sm">
      ...
      {session.has_final_video && (
        <a href={getWorkDownloadUrl(sessionId!)}
          className="inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-primary-foreground hover:bg-[#E84A4F] transition-all font-medium shadow-sm hover:shadow-md active:scale-[0.98]">
